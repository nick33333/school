...
ranger = list(range(len(yList)))
print(ranger)
# sub_idx = random.sample(ranger,1000)
# sub_yList = [yList[i] for i in sub_idx]

for xBin in np.arange(0,12,1): # This determines number of swarms... kinda
	if xBin in dataDict:
		print(xBin)
		yListFull = dataDict[xBin]
		yList=random.sample(dataDict[xBin],1000)
		start=time.time()
		...
		median=np.median(yListFull)
		panelCenter.plot([xBin-(binsize/2)/2,(binsize/2)/2+xBin],[median,median],
               linewidth=1, color='red',zorder=5)

x = np.linspace(0.25,11.75,500)
y = 500*[95]
line = panelCenter.plot(x,y, "--",linewidth=0.8, color='black', dashes=[7,8,3,8])
...
plt.savefig("Vollmers_swarm.png", dpi=600)
