...
void BinHeap :: swim(int index)
{
    if(index < 1) // BAD, shouldn't happen
        return;
    // >= FOR MIN HEAP???? if A[par] >= A[child], swap
...
// HEAPIFY AND EXTRACTMAX IMPLEMENTATIONS
void BinHeap::heapify(int idx) {
    if ((idx*2 > size)) { // Condition checks if A[idx] is a leaf by checking the index of its expected left child (lower index than right child)
//         cout<<"LEAF"<<endl;
        return;
    }
//     int idxMax = max(A[idx*2], A[(idx*2)+1]); // Finds max of left and right child of A[idx]
    int idxMax = (A[idx*2] > A[(idx*2)+1])? idx*2 : (idx*2)+1; // Finds index of the higher value between the left and right children of A[idx]
    if (A[idx] >= A[idxMax]) { // A[idx] satisfies heap invariant
//         cout<<"HEAP ALREADY"<<endl;
        return;
    } 
    if (A[idx] < A[idxMax]) { // Heapify condition for Left child (A[idx*2])
//         cout<<"HEAPIFIED SOMETHING"<<endl;
//         cout<<"idx: "<<idx<< " idxMax: "<<idxMax<<endl;
        int tmp = A[idx];
        A[idx] = A[idxMax];
        A[idxMax] = tmp;
//         cout<<"heapify print: "<<print()<<endl;
        heapify(idxMax);
        return;
    }
}

// void BinHeap::minheapify(int idx)
// {
    
// }

void BinHeap::extractMax(){
//     cout<<"EXTRACTED MAX: "<<A[1]<<endl;
//     int tmp = A[1];
    A[1] = A[size];
    A[size] = 0;
    if (size>0) {
    ...
    cout<<"size: "<<size<<" EM print: "<<print()<<endl;
    heapify(1);
} 